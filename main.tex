...
Let $D_i$ be the failure time of interest that could either be a terminal event (e.g., death)
or a non-terminal event (e.g., treatment failure).
Let $C_i$ be the potential censoring time for reasons other than the failure event
(e.g., study dropouts).
The observed data are independent and identically distributed copies
$\{N_i(t), Y_i, X_i; t\le Y_i, i = 1, \ldots, n\}$,
where $Y_i = \min(D_i, C_i)$, $\Delta_i = I(D_i\le C_i)$,
