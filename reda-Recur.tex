The \texttt{Recur()} function prepares the recurrent event data into a
\texttt{Recur} object used in the packages \pkg{reda} and \pkg{reReg}.
...
key functions in \pkg{reda} and \pkg{reReg}. The following commands
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{with}\NormalTok{(readmission[}\DecValTok{1}\OperatorTok{:}\DecValTok{14}\NormalTok{,], }\KeywordTok{Recur}\NormalTok{(t.stop, id, event, death))}
\end{Highlighting}
\end{Shaded}
